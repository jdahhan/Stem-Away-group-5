...
parsed_files = []
fnames = ["pubmed21n" + str(num).zfill(4) + ".xml" for num in range(1000,1000+SIZE + 1)]
...
for fname in fnames:
    ...
    print('yoinked')
    medline_data = pd.DataFrame.from_dict(pp.parse_medline_xml(gzip.decompress(r.content)))
    medline_data = medline_data[['abstract', 'pubdate']]
    parsed_files.append(medline_data[:50])
    print('parsed')

#%% create dataframe of parsed files
pubmed_df = pd.concat(parsed_files)
print(pubmed_df['pubdate'][10])
#%% isolate sentences
#pull abstracts <= 2015
...
for sentence in sentences:
    token_sentence = set(tokenize.tokenize(sentence))
    for drug in drugs:
        if drug in sentence:
            for gene in genes:
                if gene in sentence:
